...
Node::Node(NodeKind _kind, decimal _value) : kind(_kind), value(_value), a_var(false), b_var(false){};
Node::Node(int _varId, string _varName) : kind(NodeVar), varId(_varId), varName(_varName), a_var(true), b_var(true){};
Node::Node(NodeKind _kind, NodePtr _a) : kind(_kind), a(move(_a)), a_var(a.get()->a_var || a.get()->b_var), b_var(false){};
Node::Node(NodeKind _kind, NodePtr _a, NodePtr _b) : kind(_kind), a(move(_a)), b(move(_b)), a_var(a.get()->a_var || a.get()->b_var), b_var(b.get()->a_var || b.get()->b_var){};
...
TokenIter::TokenIter(vector<Token> _tokens, size_t _cur) : cur(_cur), tokens(_tokens){};

bool TokenIter::next(const TokenKind kind) { return cur < tokens.size() && tokens[cur].kind == kind; }

bool TokenIter::take(TokenKind kind)
{
  if (next(kind))
  {
    cur++;
...
Token TokenIter::token() { return tokens[cur - 1]; }

void TokenIter::back() { cur--; }
...
range::range(double _min_val, double _max_val) : min_val(_min_val), max_val(_max_val){}; // l1, u1 inclusive
